...
def get_content(filename):
    ...
    labels = []
    try:
        reader = codecs.open(filename, 'r' ,'utf-8')
        ...
        while line:
            ...
            if len(tokens) <> 5:
                ...
                line = reader.readline()
                continue
            cut_content = tokens[1].split(" ")
            # sen_len = len(cut_content)
            # if sen_len > max_len:
            #     max_len = sen_len

            # if len(cut_content) < 10:
            #     line = reader.readline()
            #     continue

            # content.append(cut_content[:MAX_LENGTH])
            content.append(cut_content)
            index.append(tokens[0])

            labels.append(tokens[4])
            publish_date.append(tokens[2])
            line = reader.readline()
        return index,content,labels,publish_date
...
def get_cutContent():
    content, title = get_content()    # 连接数据库获取股票研报content和title
...
posDict = deal_sentimentDict('lexicon/positive.txt')
negDict = deal_sentimentDict('lexicon/negative.txt')
negatiDict = deal_sentimentDict('lexicon/negaDict.txt')
...
index,content,labels,publish_date = get_content('data/shzhengopen_test1.txt')
...
#     if (neg[i] > -0.3) and (pos[i] < 0.3) or (-1*neg[i] == pos[i]):
#         neu += 1
#         flags.append('0')
#     elif pos[i] >= 0.3 and -1*neg[i] < pos[i]:
#         posc += 1
#         flags.append('1')
#     elif pos[i] < -1*neg[i] and neg[i] <= -0.3:
#         negc += 1
#         flags.append('2')
#     print(neg[i],pos[i])
# print (neg)
...
reader = codecs.open('data/shzhengopen_test1.txt','r','utf-8')
writer = codecs.open('data/sh_senti_test.txt','w','utf-8')
line = reader.readline()
i = 0
while line:
